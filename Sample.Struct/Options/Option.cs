...
public static class Option
{
    ...
    public static Option<T, Option<T>> AsOption<T>([CanBeNull] this T reference) where T : class =>
        new Option<T>(reference).AsOption();

    public static Option<T, Option<T>> AsOption<T>(this Option<T> option) where T : class => new(option);
    public static Option<T, TO> AsOption<T, TO>(this Option<T, TO> option) where TO : IOption<T> => option;
    public static Option<T, Null<T>> Null<T>() => Options.Null<T>._instance;
    public static Option<T, Null<T>> Null<T>(this T _) => Null<T>();

    [CanBeNull]
    public static T AsReference<T>(this Option<T> option) where T : class =>
        option.TryGetValue(out var reference) ? reference : null;
...
public readonly struct Option<T> : IOption<T>, IEquatable<Option<T>> where T : class
{
    ...
    public static implicit operator Option<T>(T reference) => new(reference);
